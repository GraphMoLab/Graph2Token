...
from datasets_utils import mol_to_graph_data_obj_simple, smi_to_graph_data_obj_simple
...
class ChemBI_HMDB_Datasets_Graph(InMemoryDataset):
    def __init__(self, root, transform=None, pre_transform=None, pre_filter=None):
        ...
        self.CID2text_file = os.path.join(self.root, "raw/graph2text_filter_none.csv")
    ...
    def process(self):
        with open(self.CID2text_file, 'r') as f:
            # 创建一个pandas的TextFileReader对象，用于逐行读取
            reader = pd.read_csv(f, iterator=True, chunksize=1)

            CID_list, graph_list, text_list = [], [], []
            # 逐行读取
            for index, chunk in enumerate(reader):
                # chunk是一个DataFrame对象，包含当前chunksize行的数据
    ...
    def load_Graph_CID_and_text(self):
        ...
        CID_text_df = pd.read_csv(self.CID2text_file)
...
class PubChem_Datasets_Only_SMILES(Dataset):
...
class PubChem_Datasets_SMILES(Dataset):
...
class PubChem_SubDatasets_SMILES(PubChem_Datasets_SMILES):
...
class PubChem_Datasets_Graph(InMemoryDataset):
    def __init__(self, root, transform=None, pre_transform=None, pre_filter=None):
        self.root = root
        self.transform = transform
        self.pre_transform = pre_transform
        ...
        super(PubChem_Datasets_Graph, self).__init__(root, transform, pre_transform, pre_filter)
...
class PubChem_SubDatasets_Graph(PubChem_Datasets_Graph):
...
class PubChem_Datasets_SMILES_and_Graph(InMemoryDataset):
    def __init__(self, root, subset_size=None, transform=None, pre_transform=None, pre_filter=None):
        ...
        super(PubChem_Datasets_SMILES_and_Graph, self).__init__(root, transform, pre_transform, pre_filter)
